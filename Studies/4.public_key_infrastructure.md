...
It is called `self-signed` because the properties `Issuer`(Issued By) and `Subject`(Issued To) are the same.

Self-signed certificates are mostly used for development purposes to run a webserver on localhost, 
or for ease of use, to host services on a home lab inside a private network.

To create a self-signed cert, first generate a RSA or ECDSA private key:
```
