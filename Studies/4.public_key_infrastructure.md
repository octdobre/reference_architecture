# :e-mail: Digital Certificates x509 (RFC 5280) and Public-Key Infrastructure (PKI)
...
It is called 'self-signed' because it's 'Issued By' and 'Issued To' properties are the same.

Self-signed certificates are used mostly for development purposes to run a webserver on your localhost 
...
Generate the RSA private key and certificate in one go:
```
...
The above configuration can be used with a few tweaks.
Replace the extensions section with this:
```
[ v3_ca ]
# must identify as CA and path0 so issued certificates cannot issue more
...
```
openssl req -x509 -config myrootca.cnf -nodes -keyout rsarootca.pri -out rootca.cer
```
...
#insert the the actual DNS entries
[ sans ]
DNS.0 = localhost
DNS.1 = 127.0.0.1
```
...
```
openssl req -new -config certreq.cnf -nodes -keyout certreq.pri -out request.csr
```

Sign Certificate Request (Sign CSR)(And the extensions also from the CSR)
```
openssl x509 -req -in request.csr -days 730 -CA rootca.cer -CAkey rsarootca.pri -CAcreateserial -out webssl.cer -extensions req_ext -extfile certreq.cnf
```


## OpenSSL configuration summarized
