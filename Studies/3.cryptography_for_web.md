# Cryptography for the web : OpenSSL
...
Create a text file name 'input.txt' for this tutorial with this contents:
```
...
```
openssl sha1 -hex -out <output file with checksum>  <input file for checksum>
...
```
---
## Confidentiality (Encryption/Decription) 

A cypher function is used protect the contents of the file.

When a cypher function is applied to a file, the contents of the file are mixed and become impossible understandable.
...
to its original understandable form.
...
This tables shows the difference between a normal file and its encrypted value:

|          | Value                                       | Encrypted value hex                             | 
|-         |-                                            |-                                                |
| AES      | The quick brown fox jumps over the lazy dog | U2FsdGVkX1/9GXspS3rhoOc6TfkeWAKBIe+cfahm1ELcK2mI6xvxn1CiRv7tbrxM
588Uwvly36HPhrliLada3Q== |
...
Encription using AES (Symetric) (-e for encrypt) (-d for decrypt)
```
...
des-ede3-cbc (-a flag encode in base64)
...
Generate Asymmetric Private Key using (RSA):
```
...
Note: A RSA cipher can never encrypt more data than the key size in bytes.
...
Generate private-public key pair as above.

Converting from PEM to DER (from PEM(Privacy Enhanced Mail) to binary format DER(Distinguished Encoding Rules))
```
...
This format does not specify which kind of key it is.
...
### Generating DSA (Digital Signature Algorithm) key-pair
...
### Using PSS padding scheme for signatures with RSA private key:
\
...
```
type mackey & type input.txt | openssl sha256 -out input.txt.macsig
```
...
```
openssl sha1 -hmac -in hmackey -out input.txt.hmacsig input.txt

openssl sha256 -hmac -in hmackey -out input.txt.hma256csig input.txt
```
